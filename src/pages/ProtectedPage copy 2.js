...
const ProtectedPage = () => {
  ...
  const initialValues = {
    nombre: '',
    telefono: '',
    email: user ? user.mail || '' :'', // sie el usuario esta definido tomamos su mail si no se agrega a mano
    reciboSueldo: null,
    dniFrente: null,
    dniDorso: null,
  ...
  const handleSubmit = async (values) => {
    ...
    formData.append('mail', values.email);  // Ahora tomamos el email ingresado manualmente.
    ...
    try {
      ...
      if (error.response) {
        ...
        alert(`Error: ${error.response.status} - ${error.response.data.message || 'Error submitting form'}`);
      } else if (error.request) {
        // La solicitud se realizó pero no se recibió respuesta
        console.error('Error submitting form: No response received from server', error.request);
