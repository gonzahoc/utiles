...
function log(message){
  fs.writeFileSync(path.join(__dirname,'./tmp/logs.txt'), message+'\r\n', { flag: 'a+' });
}
...
// app.use(cors({
//   //origin: ['https://www.stipa.org.ar', 'https://stipa.org.ar','https://api.stipa.org.ar','http://www.stipa.org.ar','http://stipa.org.ar'], // Permitir ambos dominios
//   origin: ['https://stipa.org.ar:3000', 'http://stipa.org.ar:3000'], // Permitir solo el dominio de tu frontend
//   methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'], // Métodos permitidos
//   allowedHeaders: ['Content-Type', 'Authorization'], // Cabeceras permitidas
//   credentials: true // Permitir cookies de sesión y credenciales
// }));
...
// sistema sin el expressjson embebido
app.post('/api/register', express.json(), (req, res) => {
  //app.use(express.json());
  const { nombre, cuil, empresa, nrosindical, pass, mail } = req.body;
  const token = generateToken();
...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    try {
      ...
      log('Error uploading files:' + JSON.stringify(error))
    }
  },
  filename: function (req, file, cb) {
    try {
      log('Filename: '+file.fieldname);
...
app.post('/api/formulario', upload.any(), (req, res) => {
  try {
    // Sin callback
    log('Formulario started:' + JSON.stringify(req.body));
...
//app.listen(port, () => {
//s console.log(app.address());
//console.log('Server running at http://api.stipa.org.ar/');
//log('App initiated', { flag: 'a+' }, err => {});
//});
